...
//		System.out.println("Field "+f+" of type "+f.init+" added");
	...
	public boolean preGenerate() {
		...
		// setup correct super-type for classes that extends mixins
		if (this.isClazz() && this.super_types.length >= 2) {
			TypeRef st = this.super_types[0];
			if (st.isAutoGenerated() && st.getType() ≈ StdTypes.tpObject) {
				TypeDecl td = this.super_types[1].getTypeDecl();
				if (td.isClazz()) {
					((Struct)this).super_types.del(0);
				}
				else if (td instanceof Struct && td.isMixin()) {
					st = new TypeRef(((Struct)td).iface_impl.xtype);
					st.setAutoGenerated(true);
					((Struct)this).super_types[0] = st;
				}
			}
		}
			...
			foreach (Constructor m; members) {
				...
				if (ctor_call == null) {
					...
					if( super_types.length > 0 && super_types[0].getStruct() == Type.tpClosureClazz ) {
						...
						ctor_call.args.insert(0,new EToken(pos, nameImpl, ETokenKind.IDENTIFIER,true));
					}
