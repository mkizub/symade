...
public abstract class kiev040 implements kiev040Constants {
	...
	private Field mkField(Symbol id, TypeRef tp, ASTModifiers modifiers) {
		if (tp == null)
			tp = new TypeDeclRef();
		Field f = new Field(id, tp, 0);
		f.pos = id.pos;
		modifiers.copyToNode(f);
		return f;
	}

	private FieldEnum mkEnumField(Symbol id, ASTModifiers modifiers) {
		FieldEnum f = new FieldEnum(id);
		f.pos = id.pos;
	...
	private Var mkVar(Symbol id, ASTModifiers modifiers, TypeRef tp) {
		if (tp == null)
			tp = new TypeDeclRef();
	...
	private LocalRuleVar mkRuleVar(Symbol id, ASTModifiers modifiers, TypeRef tp, boolean first) {
		if (!first)
			tp = tp.ncopy();
	...
	private FormPar mkVarargPar(Symbol id, ASTModifiers modifiers, TypeRef vt) {
		FormPar v = new FormPar(id, vt, null, FormPar.PARAM_VARARGS, ACC_FINAL);
		modifiers.moveToNode(v);
		return v;
	}
 ...
 */
  ...
  ASTModifiers modifiers; DeclGroupEnums enums; SpacePtr clazz_members = clazz.getSpacePtr("members");
    jj_consume_token(LBRACE);
    if (clazz.isEnum()) {
                  enums = new DeclGroupEnums();
      modifiers = Modifiers();
  ...
  final public FieldEnum EnumFieldDeclaration(ASTModifiers modifiers) throws ParseException {
  Symbol id; FieldEnum f; ConstStringExpr text;
    id = Name();
          f = mkEnumField(id, modifiers);
    switch (jj_nt.kind) {
    case COLON:
      jj_consume_token(COLON);
      f.alt_id = StringConstExpression();
      break;
    default:
      ;
    }
  ...
  final public DNode FieldDecls(ASTModifiers modifiers) throws ParseException {
  boolean old_declMode; TypeRef type; Field f; DeclGroup decl; DNode ret;
    type = Type();
    ...
    try {
      if (jj_2_22(2147483647)) {
        ...
        case IDENTIFIER:
        case OPERATOR_AT:
                          decl = new DeclGroup(); modifiers.copyToNode(decl); decl.dtype = type; ret = decl;
          modifiers = Modifiers();
  ...
  final public LocalRuleVar RuleVarDecl(ASTModifiers modifiers, TypeRef type, boolean first) throws ParseException {
  Token t; Symbol id; LocalRuleVar lv;
    id = Name();
  ...
  final public DNode VarDecls(ASTModifiers modifiers) throws ParseException {
  boolean old_declMode; TypeRef type; DeclGroup decl;
                old_declMode = declMode;
                declMode = false;
                decl = new DeclGroup();
                modifiers.copyToNode(decl);
    try {
      decl.dtype = Type();
      modifiers = Modifiers();
  ...
  Token t; Symbol a; SymbolRef id; ASTOperator op; ConstIntExpr ce; ConstStringExpr cs;
    ...
    case ALIAS:
      ...
      if (jj_2_29(2)) {
        id = NameRef();
        jj_consume_token(OPERATOR_ID);
                          a = new ASTOperatorAlias(); a.setMode(id);
        if (jj_2_28(1)) {
          op = Operator();
                                                                          a.setImage(op);
        } else {
 ...
 */
  ...
  final private boolean jj_3R_216() {
    ...
    lookingAhead = false;
    if (!jj_semLA || jj_3R_438()) return true;
    if (jj_scan_token(IDENTIFIER)) return true;
    if (jj_3R_137()) return true;
    if (jj_3R_101()) return true;
    xsp = jj_scanpos;
  ...
  final private boolean jj_3R_405() {
    ...
    lookingAhead = false;
    if (!jj_semLA || jj_3R_429()) {
    jj_scanpos = xsp;
    lookingAhead = true;
    jj_semLA = getToken(1).kind == LBRACE;
    lookingAhead = false;
    if (!jj_semLA || jj_3R_430()) {
    jj_scanpos = xsp;
    lookingAhead = true;
