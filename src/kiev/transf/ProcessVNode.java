...
public final class ProcessVNode extends TransfProcessor implements Constants {
	...
	private void autoGenerateMembers(Struct:ASTNode s) {
		...
		if (hasMethod(s, nameEnumValues)) {
			...
			Method elems = new Method(nameEnumValues,et.ret,ACC_PUBLIC);
			s.addMethod(elems);
			...
			Method getV = new Method(KString.from("getVal"),getVt.ret,ACC_PUBLIC);
		...
		if (!mnMeta.getZ(nameCopyable) || s.isAbstract()) {
			...
			Method copyV = new Method(KString.from("copy"),copyVt.ret,ACC_PUBLIC);
			s.addMethod(copyV);
		...
		if (hasMethod(s, KString.from("copyTo"))) {
			...
			Method copyV = new Method(KString.from("copyTo"),copyVt.ret,ACC_PUBLIC);
		...
		if (hasMethod(s, KString.from("setVal"))) {
			...
			Method setV = new Method(KString.from("setVal"),setVt.ret,ACC_PUBLIC);
