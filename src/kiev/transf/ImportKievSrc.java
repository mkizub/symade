...
public final class KievFE_Pass1 extends TransfProcessor {
	...
	private void processFileHeader(FileUnit fu) {
		...
		foreach (SNode n; fu.members) {
			try {
				processSyntax(n);
				if (n instanceof Import && n.mode == Import.ImportMode.IMPORT_SYNTAX)
					any_syntax = true;
			} catch(Exception e ) {
		...
		if( !any_syntax ) {
			Import imp = new Import(Env.newStruct("Syntax",Env.newPackage("kiev.stdlib"),ACC_SYNTAX,new KievSyntax()),false);
