...
public final class KievFE_Pass1 extends TransfProcessor {
	...
	public void processSyntax(ImportSyntax:ASTNode astn) {
		if (!Env.needResolving(astn.name))
			return;
		if (!astn.name.qualified)
			astn.name.qualified = true;
	...
	next_super_syntax:
		foreach(SymbolRef sr; astn.super_syntax; Env.needResolving(sr)) {
			...
			while (dot > 0) {
				...
				head = name.substring(0,dot).intern();
				name = name.substring(dot+1).intern();
				ResInfo<KievPackage> info = new ResInfo<KievPackage>(env,astn,head,ResInfo.noForwards|ResInfo.noSuper|ResInfo.noSyntaxContext);
				if!(scope.resolveNameR(info)) {
			...
			ResInfo<KievSyntax> info = new ResInfo<KievSyntax>(env,astn,name,ResInfo.noForwards|ResInfo.noSuper|ResInfo.noSyntaxContext);
			if!(scope.resolveNameR(info)) {
...
public final class KievFE_Pass2 extends TransfProcessor {
	...
	public void doProcess(SyntaxScope:ASTNode astn) {
		foreach (ASTNode n; astn.members) {
			try {
	...
	public void doProcess(KievSyntax:ASTNode astn) {
		KievSyntax stx = astn;
...
public final class KievFE_MetaDecls extends TransfProcessor {
	...
	public void doProcess(SyntaxScope:ASTNode node) {
		foreach(ASTNode n; node.members)
			doProcess(n);
	}
...
public final class KievFE_MetaDefaults extends TransfProcessor {
	...
	public void doProcess(SyntaxScope:ASTNode fu) {
