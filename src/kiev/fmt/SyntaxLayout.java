...
public abstract class ATextSyntax extends DNode implements ScopeOfNames, GlobalDNode {
	...
	public boolean preResolveIn() {
		if (parent_syntax.name != null && parent_syntax.name != "") {
			...
			else if (parent_syntax.symbol != ts.id) {
				parent_syntax.open();
...
public final class SyntaxElemDecl extends ASyntaxElemDecl {
	...
	public SyntaxElemDecl(Struct cls, SyntaxElem elem) {
		...
		this.rnode = new SymbolRef<Struct>((Symbol<Struct>)cls.id);
	}

	public void preResolveOut() {
		if (rnode == null)
			rnode = new SymbolRef<Struct>(0,(Symbol<Struct>)Env.newStruct("ASTNode", Env.newPackage("kiev.vlang"), 0, null).id);
		if (rnode.name == null)
			rnode.name = "ASTNode";
		Struct@ s;
...
public final class SyntaxFunction extends ASTNode {
	...
	@att public SyntaxFuncActions	act;
	@att public String				attr;

	public SyntaxFunction() {
		act = SyntaxFuncActions.FuncNop;
	}
}
...
public final class SyntaxElemRef extends SyntaxElem {
	...
	public SyntaxElemRef(ASyntaxElemDecl decl) {
		this.decl = new SymbolRef<ASyntaxElemDecl>((Symbol<ASyntaxElemDecl>)decl.id);
	}
...
public abstract class SyntaxAttr extends SyntaxElem {
	...
	public SyntaxAttr(String name, ATextSyntax stx) {
		this.name = name;
		this.in_syntax = new SymbolRef<ATextSyntax>((Symbol<ATextSyntax>)stx.id);
		this.par = new SymbolRef<AParagraphLayout>();
	}
...
public final class CalcOptionNotNull extends CalcOption {
	...
	public boolean calc(ANode node) {
		...
		if (obj instanceof SymbolRef && obj.name == null)
			return false;
		if (obj instanceof Symbol && obj.sname == null)
			return false;
		return true;
	}
}
...
@node
public class CalcOptionHasMetaFlag implements CalcOption {
	@virtual typedef This  = CalcOptionHasMetaFlag;

	public CalcOptionHasMetaFlag() {}
	public CalcOptionHasMetaFlag(String name) {
		super(name);
	}
...
@node
public class CalcOptionHasUserMeta implements CalcOption {
	@virtual typedef This  = CalcOptionHasUserMeta;

	public CalcOptionHasUserMeta() {}
	public CalcOptionHasUserMeta(String name) {
		super(name);
	}
...
public class SyntaxIntChoice extends SyntaxAttr {
	...
	public SyntaxIntChoice(String name) {
		super(name);
	}
...
public class SyntaxParagraphLayout extends SyntaxElem {
	...
	public SyntaxParagraphLayout(SyntaxElem elem, AParagraphLayout par) {
		this.elem = elem;
		this.par = new SymbolRef<AParagraphLayout>(par.id);
	}
