	...
	public void preFormat(DrawContext cont) {
		...
		if (narr == null || narr.length == 0) {
			...
			if (slst.empty != null) {
				prefix = null;
				sufix = null;
				args.append(slst.empty.makeDrawable(cont.fmt, null, text_syntax));
				args[0].preFormat(cont,slst.empty,null);
			} else {
				if (prefix == null && slst.prefix != null)
					prefix = slst.prefix.makeDrawable(cont.fmt, node, text_syntax);
				if (sufix == null && slst.sufix != null)
					sufix = slst.sufix.makeDrawable(cont.fmt, node, text_syntax);
				if (prefix != null)
					prefix.preFormat(cont,slst.prefix,null);
				if (sufix != null) 
					sufix.preFormat(cont,slst.sufix,null);
			}
		} else {
			if (prefix == null && slst.prefix != null)
				prefix = slst.prefix.makeDrawable(cont.fmt, node, text_syntax);
