...
public final class Draw_Layout {
...
public final class Draw_SyntaxFunction {
...
public abstract class Draw_CalcOption {
...
public abstract class Draw_SyntaxAttr extends Draw_SyntaxElem {
	...
	public SymbolRef[]						expected_types = SymbolRef.emptyArray;
...
public class Draw_SyntaxXmlStrAttr extends Draw_SyntaxAttr {
	...
	public Draw_SyntaxXmlStrAttr(String name) {
		this.name = name;
	}
	public Drawable makeDrawable(Formatter fmt, ANode node, Draw_ATextSyntax text_syntax) {
...
public class Draw_SyntaxXmlTypeAttr extends Draw_SyntaxAttr {
	public Draw_SyntaxXmlTypeAttr() {}
	public Draw_SyntaxXmlTypeAttr(String name) {
		this.name = name;
	}
	public Drawable makeDrawable(Formatter fmt, ANode node, Draw_ATextSyntax text_syntax) {
...
public class Draw_SyntaxElemDecl {
	public Draw_SyntaxElem					elem;
	public String 							clazz_name;
}

public class Draw_SyntaxIdentTemplate {
...
public class Draw_SyntaxExprTemplate {
	public Draw_SyntaxElem					elem;
...
public class Draw_SyntaxJavaCommentTemplate {
	public Draw_SyntaxElem					elem;
...
public class Draw_ATextSyntax {
	protected Hashtable<String,Draw_SyntaxElem>					badSyntax = new Hashtable<Class,Draw_SyntaxElem>();
	protected Hashtable<String,Draw_SyntaxElemDecl>				allSyntax = new Hashtable<String,Draw_SyntaxElemDecl>();
	protected Hashtable<Pair<Operator,Class>, Draw_SyntaxElem> 	allSyntaxExprs = new Hashtable<Pair<Operator,Class>, Draw_SyntaxElem>();

	public Draw_ATextSyntax				parent;
	...
	public Draw_SyntaxElem getSyntaxElem(ANode for_node) {
		if (for_node != null) {
			...
			Draw_SyntaxElemDecl sed = allSyntax.get(cl_name);
			if (sed != null)
				return sed.elem;
		}
		...
		if (parent != null)
			return parent.getSyntaxElem(for_node);
		Draw_SyntaxElem se;
		if (for_node == null) {
...
public class Draw_XmlDumpSyntax extends Draw_ATextSyntax {
	...
	public Draw_SyntaxElem getSyntaxElem(ANode node) {
		...
		foreach (AttrSlot attr; node.values(); attr != ASTNode.nodeattr$this && attr != ASTNode.nodeattr$parent) {
			...
			if (attr.is_space) {
				...
				if (ANode.class.isAssignableFrom(attr.clazz))
					...
					se = set(open0(attr.name), new Draw_SyntaxXmlStrAttr(attr.name), close0(attr.name));
				else if (attr.clazz == Operator.class)
					se = set(open0(attr.name), new Draw_SyntaxXmlStrAttr(attr.name), close0(attr.name));
				else if (Type.class.isAssignableFrom(attr.clazz))
					se = set(open0(attr.name), new Draw_SyntaxXmlTypeAttr(attr.name), close0(attr.name));
		...
		Draw_SyntaxElemDecl sed = new Draw_SyntaxElemDecl();
		sed.elem = ss;
		...
		if (node != null) {
			...
			if (sed != null) {
				...
				if (node instanceof ENode && se instanceof Draw_SyntaxExpr) {
					...
					if (se == null) {
						se = expr(op, (Draw_SyntaxExpr)sed.elem);
