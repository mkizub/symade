...
public final class DumpUtils {
	...
	public static FileUnit loadFromXmlData(byte[] data, String tdname, DNode pkg) {
		assert (Thread.currentThread() instanceof WorkerThread);
		XMLDeSerializer deserializer = new XMLDeSerializer();
					...
					if (pkg != null) {
						...
						fu.scanned_for_interface_only = true;
						{
							...
							while (p != null && !(p instanceof KievPackage))
								p = p.parent();
							fu.srpkg.symbol = (KievPackage)p;
						}
						root = fu;
						DNode td = Env.getRoot().resolveGlobalDNode(qname);
						if (td != null)
							assert(td.getClass().getName().equals(cl_name));
						else if (lng != null)
							td = (DNode)lng.makeNode(elName);
						else
							td = (DNode)Class.forName(cl_name).newInstance();
						...
						if (td instanceof KievPackage) {
							...
							if (td.parent() == null)
								((KievPackage)pkg).pkg_members += td;
						} else {
							fu.members += td;
							fu.getPackage().pkg_members += td;
						}
						if (td instanceof Struct) {
							td.initStruct(tdname, 0);
						}
