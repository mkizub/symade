...
public class Attr implements Constants {
	...
	public boolean get$isKiev() { return false; }
	public void set$isKiev(boolean b) { return; }
...
public class ClassArgumentsAttr extends Attr {
	...
	public void generate() {
		...
		for(int i=0; i < args.length; i++) {
			...
			ConstPool.addAsciiCP(args[i].clazz.super_clazz.signature);
		}
	}
	...
	public kiev.bytecode.Attribute write() {
		...
		for(int i=0; i < args.length; i++) {
			...
			kaa.cp_supername[i] = ConstPool.getAsciiCP(args[i].clazz.super_clazz.signature).pos;
			kaa.argno[i] = argno[i];
		}
		return kaa;
	}
	...
	public void write_annotation(Meta m, kiev.bytecode.Annotation.annotation a) {
		...
		foreach (MetaValue v; m) {
			...
			if (v instanceof MetaValueScalar) {
				...
				a.values[n] = write_values(mva.values);
			}
			n++;
		}
	}
...
public class DefaultMetaAttr extends MetaAttr {
	...
	public kiev.bytecode.Attribute write() {
		...
		if (mv instanceof MetaValueScalar)
			...
			a.value = write_values(((MetaValueArray)mv).values);
		return a;
	}
}
