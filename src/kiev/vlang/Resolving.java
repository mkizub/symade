...
public final class ResInfo<D extends DNode> implements Cloneable {
	...
	private INode		space_prev_node;
	private AttrSlot	space_prev_slot;
	public Vector<ANode> processedNS;
	...
	public ResInfo(Env env, ANode from, String nm) {
		this(env, from, nm, 0);
	}
	public ResInfo(Env env, ANode from, String nm, int fl) {
		this.env = env;
		if (nm != null) {
			assert (nm.intern() == nm);
			this.name = nm;
		...
		try {
			if (from instanceof TypeDecl)
				...
				from_scope = Env.ctxTDecl(from);
		} catch (Throwable t) {}
		this.processedNS = new Vector<ANode>();
	}
	
	public String getName() { return name; }
	public ANode getFrom() { return from; }
	
	public void setPrev(INode node) {
		space_prev_node = node;
		space_prev_slot = node.pslot();
	}
	public INode getPrevNode() {
		return space_prev_node;
	}
	public AttrSlot getPrevSlot() {
		return space_prev_slot;
	}
	public String getPrevSlotName() {
		if (space_prev_slot == null)
			return null;
		return space_prev_slot.name;
	}
	
	public void addProcessedNS(ANode p) {
		if (processedNS.contains(p))
			return;
		processedNS.append(p);
	}
	
	public boolean isProcessedNS(ANode p) {
		return processedNS.contains(p);
	}
	...
	private boolean check(ISymbol isym) {
		...
		if (n instanceof Var)
			...
			if (d.isPrivate()) {
				...
				ComplexTypeDecl p = Env.ctxTDecl(n);
				while (s != null && s != p) {
					ComplexTypeDecl x = Env.ctxTDecl(s);
					if (x == null || !x.isClazz())
						break;
					s = x;
				}
	...
	private ENode buildAccess(ASTNode at, ASTNode from, Object node, boolean node_is_generated) {
		...
		if (from == null && isEmpty()) {
			...
			if (node instanceof Field) {
				...
				if (parent instanceof ComplexTypeDecl && parent.isSingleton()) {
					ENode acc = new AccFldExpr(at.pos,null,parent.resolveField(Env.getEnv(),Constants.nameInstance));
					if (node_is_generated)
						acc.setAutoGenerated(true);
					ENode ret = new AccFldExpr(at.pos,acc,(Field)node);
					if (node_is_generated)
						ret.setAutoGenerated(true);
					return ret;
				}
				if (Env.ctxMethod(at) == null) {
					ThisExpr te = new ThisExpr(at.pos);
					te.setAutoGenerated(true);
					return buildAccess(at, te, node, node_is_generated);
				}
		...
		if (from != null) {
			...
			if (from instanceof TypeRef) {
				...
				if (isEmpty() && node instanceof Field) {
					...
					if (from.getTypeDecl(Env.getEnv()).isSingleton()) {
						ENode acc = new AccFldExpr(at.pos,(ENode)from,from.getTypeDecl(Env.getEnv()).resolveField(Env.getEnv(),Constants.nameInstance));
						if (node_is_generated)
							acc.setAutoGenerated(true);
						ENode ret = new AccFldExpr(at.pos,acc,(Field)node);
						if (node_is_generated)
		...
		if (e != null && (node instanceof Field || node instanceof Type)) {
			for (; n < forwards_p; n++) {
				...
				if (fwn instanceof Type) {
					...
					e = new UnresReinterpExpr(e.pos, tr, e);
				}
	...
	public UnresCallExpr buildCall(ASTNode at, ENode from, TypeRef[] targs, ENode[] args) {
		...
		if (node instanceof Method) {
			...
			if (from == null && forwards_p == 0) {
				...
				TypeRef tr = new TypeRef(Env.ctxTDecl(meth).getType(env));
				tr.setAutoGenerated(true);
			...
			if (from == null && forwards_p == 0) {
				...
				return new UnresCallExpr(at.pos, new TypeRef(Env.ctxTDecl(f).getType(env)), resolvedSymbol(), targs, args, false);
			}
