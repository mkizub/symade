...
public final class ResInfo {
	...
	public ENode buildCall(ASTNode at, ENode from, ISymbol isym, TypeRef[] targs, ENode[] args) {
		DNode node = isym.dnode;
		if (node instanceof Method) {
			...
			if (from == null && forwards_p == 0) {
				...
				tr.setAutoGenerated(true);
				return new UnresCallExpr(at.pos, tr, isym, targs, args, false);
			}
			ENode expr = from;
			if (forwards_p > 0)
				expr = buildAccess(at, from, forwards_stack[--forwards_p], true);
			return new UnresCallExpr(at.pos, expr , isym, targs, args, false);
		}
			...
			if (from == null && forwards_p == 0) {
				...
				return new UnresCallExpr(at.pos, new TypeRef(f.ctx_tdecl.xtype), isym, targs, args, false);
			}
