 ...
 * its parent shell.  It is used for selecting items when editing in a TextEditor (similar to the
 * list that appears when you open a Combo box).
 *
 * The list will be positioned so that it does not run off the window and the largest number of items
 * are visible.  It may appear above the current cursor location or below it depending how close you 
 * are to the edge of the window.
 * 
 * The shell lacks of handle any of the keyboard or mouse events and supposed to deactivate manually 
 * from the caller. 
 *
 */
public class PopupList {
	...
	/**
	 * The pop-up list has its own shell. It does matter what shell is used.
	 */
	Shell shell;
	 ...
	 * @return <code>0</code> if not confirmed, positive otherwise
	 */
	...
	/**
	 * Launches the Popup List, waits for an item to be selected or somebody closes it down.
	 *
	 * @param rect the initial size and location of the PopupList; the dialog will be
	 *        positioned so that it does not run off the window and the largest number of items are visible
	 *
	...
	public String open(Rectangle rect) {
		...
		Rectangle windowSize = Window.getShell().getBounds();

		// Position the dialog so that it does not run off the window and the largest number of items are visible
		int spaceBelow = windowSize.y + windowSize.height - (rect.y + rect.height) - 30;
		int spaceAbove =  rect.y - windowSize.y - 30;
