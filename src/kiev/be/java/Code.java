...
//						+" is "+stack_at(m.params.length-i-1)+" but method "+m+" expects "+m.params[i].type.getJType());
		...
		KString sign;
		JType ttt = Type.getRealType(tp,((JStructView)m.jparent).concr_type).getJType();
...
//		labels = (CodeLabel[])Arrays.append(labels,l);
	...
	public void addInstr(Instr i, JFieldView f, Type tp) {
		...
		JType ttt = Type.getRealType(tp.getErasedType(),f.jctx_clazz.concr_type).getJType();
		KString struct_sig = ttt.java_signature;
		KString field_sig = Type.getRealType(f.jctx_clazz.concr_type,f.type).getJType().java_signature;
